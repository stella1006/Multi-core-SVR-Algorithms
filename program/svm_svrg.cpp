...
double t_=0.0;
string outname="./svm_svrg_"+to_string(CORE)+".csv";
...
int main() {
    ...
    for (int iter=1; iter<=ITERATION; iter++) {
        ...
        for (int t=1; t<=M; t++) {
            ...
            for (unsigned int j=0; j<ins.features.size(); j++) {
                weights[ins.features[j].index]+=-RATE*(gra1[ins.features[j].index]-gra2[ins.features[j].index]+est_miu[ins.features[j].index]);
            }
